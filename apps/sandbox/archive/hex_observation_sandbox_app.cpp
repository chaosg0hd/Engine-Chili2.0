#include "hex_observation_sandbox_app.hpp"
...
namespace
{
    ...
    constexpr unsigned char kToggleHexDebugGridKey = 'G';
    ...
    constexpr float kHexObservationDebugLineThickness = 0.0035f;
...
void SandboxApp::UpdateLogic(EngineCore& core)
{
    ...
    if (m_state.hexObservationMode)
    {
        if (core.WasKeyPressed(kToggleHexDebugGridKey))
        {
            m_state.showHexObservationDebugGrid = !m_state.showHexObservationDebugGrid;
        }
...
FramePrototype SandboxApp::BuildLightLabFrame()
{
    ...
    if (m_state.hexObservationMode)
    {
        ...
        if (m_state.showHexObservationDebugGrid)
        {
            AddHexObservationDebugGrid(view);
        }
...
std::wstring SandboxApp::BuildOverlay(const EngineCore& core) const
{
    if (m_state.hexObservationMode)
    {
        return
            ...
            L"  Debug Grid: " + std::wstring(m_state.showHexObservationDebugGrid ? L"On" : L"Off") + L"\n"
            L"  H: toggle hex observation\n"
            L"  G: toggle hex debug grid\n"
...
void SandboxApp::AddHexObservationPatch(ViewPrototype& view, const HexObservationCell& cell) const
{
    ...
    item.screenPatch.centerX = cell.ndcX;
    item.screenPatch.centerY = cell.ndcY;
    item.screenPatch.halfWidth = std::max(0.012f, cell.ndcRadius * 0.72f);
    item.screenPatch.halfHeight = std::max(0.012f, cell.ndcRadius * 0.72f * aspect);
    item.screenPatch.color = BuildHexObservationAggregateColor(cell);
    view.items.push_back(item);
}

void SandboxApp::AddHexObservationDebugGrid(ViewPrototype& view) const
{
    for (const HexObservationCell& cell : m_hexObservationCells)
    {
        if (!cell.visible)
        ...
        const std::uint32_t color =
            cell.active
                ...
                : (cell.settled
                    ? 0xFF7D8790u
                    : (cell.sampled ? 0xFFE8F7FFu : 0xFF2A6478u));
        ...
        for (unsigned int index = 0U; index < 6U; ++index)
        {
            const float angle = 0.523598776f + (static_cast<float>(index) * 1.04719755f);
            const float vertexX = cell.ndcX + (std::cos(angle) * cell.ndcRadius);
            const float vertexY = cell.ndcY + (std::sin(angle) * cell.ndcRadius);
            if (index == 0U)
            {
                firstX = vertexX;
                firstY = vertexY;
            ...
            else
            {
                AddScreenSpaceLine(
                    ...
                    vertexX,
                    vertexY,
                    kHexObservationDebugLineThickness,
                    color);
            }
...
void SandboxApp::AddScreenSpaceLine(
    ViewPrototype& view,
    float startX,
    float startY,
    float endX,
    ...
    std::uint32_t color) const
{
    ...
    ItemPrototype item;
    item.kind = ItemKind::ScreenPatch;
    item.screenPatch.centerX = (startX + endX) * 0.5f;
    item.screenPatch.centerY = (startY + endY) * 0.5f;
    item.screenPatch.halfWidth = length * 0.5f;
    item.screenPatch.halfHeight = thickness;
    item.screenPatch.rotationRadians = std::atan2(deltaY, deltaX);
